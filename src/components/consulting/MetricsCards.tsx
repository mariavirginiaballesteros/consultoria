  ...
  return (
    <div className={`grid gap-4 mb-8 ${isClientView ? 'grid-cols-2 md:grid-cols-3' : 'grid-cols-2 md:grid-cols-4'}`}>
      <Card className="shadow-md border-slate-100 rounded-xl">
        <CardContent className="p-5">
          <div className="text-[11px] text-slate-500 uppercase tracking-wider mb-2 font-bold">
            {isClientView ? 'Horas registradas' : 'Total horas'}
          </div>
          <div className="text-3xl font-black text-[#2A2B73]">{totalHours}h</div>
          {!isClientView && (
            <div className="text-[11px] text-slate-400 mt-1 font-medium">
              {monthlyHours > 0 ? `vs ${monthlyHours}h presupuesto` : 'Sin abono límite'}
            </div>
          )}
        </CardContent>
      </Card>

      {isClientView && monthlyHours > 0 && (
         <Card className="shadow-md border-slate-100 rounded-xl">
           <CardContent className="p-5">
      ...
      {isClientView && monthlyHours === 0 && (
         <Card className="shadow-md border-slate-100 rounded-xl">
           <CardContent className="p-5">
             <div className="text-[11px] text-slate-500 uppercase tracking-wider mb-2 font-bold">Modalidad</div>
             <div className="text-xl font-black text-[#62BAD3] leading-tight mt-2">Por Servicios</div>
           </CardContent>
         </Card>
      )}
